...
const EditProfileModal: React.FC<Props> = ({ open, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    if (website.trim()) {
      const websiteRegex = /^www\.[^\s]+\.(com|org|net|edu|gov|mil|info|biz|xyz|online|io|ai|tech|app|dev)(\.[a-z]{2,3})?$/i;
    ...
    try {
      ...
      if (address2.trim() !== user?.address2) businessPayload.address2 = address2.trim();
      if (city.trim() !== user?.city) businessPayload.city = city.trim();
      if (usState.trim() !== user?.us_state) businessPayload.us_state = usState.trim();
      if (postCode.trim() !== user?.post_code) businessPayload.post_code = postCode.trim();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/30 p-4">
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-xl relative">
        ...
        <div className="px-6 mt-3 flex flex-col items-center">
          <img src={avatarPreview} alt="avatar" className="h-24 w-24 rounded-full object-cover ring-2 ring-white shadow" />
          <button type="button" onClick={onPickFile} className="mt-2 px-3 py-2 rounded-full bg-secondary-blue text-white flex items-center gap-1 text-xs">
            <FiCamera /> Change Photo
          </button>
          <input ref={inputFileRef} type="file" accept="image/*" className="hidden" onChange={onFile} />
        </div>

        {/* Form */}
        <div className="px-6 pb-6 pt-4 space-y-5 overflow-y-auto max-h-[70vh]">
          <Field icon={<FiUser />} label="Full Name" required value={name} onChange={setName} />

          {/* Phone with Country Dropdown */}
          <PhoneField country={country} setCountry={setCountry} localPhone={localPhone} setLocalPhone={setLocalPhone} openCountry={openCountry} setOpenCountry={setOpenCountry} />

          <Field icon={<FiMail />} label="Email" value={user?.email || ""} disabled />

          <hr className="my-4" />

          <Field icon={<FiBriefcase />} label="Company Name" value={companyName} onChange={setCompanyName} />
...
const Field = ({ icon, label, value, onChange, required, disabled }: any) => (
  <div>
    ...
    <div className="relative">
      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-primary-purple">{icon}</span>
      <input type="text" value={value} onChange={(e) => onChange?.(e.target.value)} disabled={disabled} className={`w-full pl-12 pr-4 py-3 rounded-full border text-sm ${disabled ? "bg-gray-50 text-gray-500" : "bg-white"}`} />
    </div>
  </div>
);

// 🔹 DropdownField
const DropdownField = ({ label, icon, open, setOpen, options, value, setValue }: any) => (
  <div>
    <label className="block text-xs font-medium mb-2 text-primary-blue">{label}</label>
    <div className="relative">
      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-primary-purple">{icon}</span>
      <button type="button" onClick={() => setOpen((s: boolean) => !s)} className="w-full pl-12 pr-10 py-3 rounded-full border bg-white text-sm text-gray-800 text-left">
        {value || `Select ${label}`}
      </button>
      {open && (
        <ul className="absolute z-40 mt-1 w-full bg-white border rounded-2xl shadow-lg max-h-44 overflow-auto text-sm">
          {options.map((opt: string) => (
            <li key={opt}>
              <button type="button" onClick={() => { setValue(opt); setOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-primary-purple/10">
                {opt}
              </button>
            </li>
          ))}
...
// 🔹 IndustryField
const IndustryField = ({ label, value, setValue, open, setOpen, search, setSearch, filtered }: any) => (
  <div>
    <label className="block text-xs font-medium mb-2 text-primary-blue">{label}</label>
    <div className="relative">
      <span className="absolute left-5 top-1/2 -translate-y-1/2">
        <img src={value ? filtered.find((s: any) => s.subcategory === value)?.icon || IndustryLogo : IndustryLogo} alt="" className="h-5 w-5" />
      </span>
      <button type="button" onClick={() => setOpen((s: boolean) => !s)} className="w-full pl-12 pr-10 py-3 rounded-full border bg-white text-sm text-gray-800 text-left">
        {value || `Select ${label}`}
      </button>
      {open && (
        <div className="absolute z-40 mt-1 w-full bg-white border rounded-2xl shadow-lg max-h-56 overflow-auto text-sm">
          <div className="sticky top-0 bg-white border-b p-2">
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search..." className="w-full px-3 py-2 rounded-full border text-sm" />
          </div>
          <ul>
            {filtered.length > 0 ? (
              filtered.map((opt: any) => (
                <li key={opt.subcategory}>
                  <button type="button" onClick={() => { setValue(opt.subcategory); setOpen(false); setSearch(""); }} className="w-full flex items-center gap-2 px-3 py-2 hover:bg-primary-purple/10">
                    <img src={opt.icon} alt="" className="h-4 w-4" /> {opt.subcategory}
                  </button>
                </li>
              ))
            ) : (
...
// 🔹 PhoneField
const PhoneField = ({ country, setCountry, localPhone, setLocalPhone, openCountry, setOpenCountry }: any) => (
  <div>
    <label className="block text-xs font-medium mb-2 text-primary-blue">Phone</label>
    <div className="relative flex">
      <button type="button" onClick={() => setOpenCountry((s: boolean) => !s)} className="pl-3 pr-8 py-3 rounded-l-full bg-white border border-gray-200 text-sm text-gray-800 flex items-center" style={{ minWidth: "100px" }}>
        {countries.find((c) => c.code === country)?.dialCode || "+--"}
        <svg xmlns="http://www.w3.org/2000/svg" className={`ml-6 h-3 w-3 text-gray-500 transition-transform ${openCountry ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      <input type="tel" placeholder="Enter Phone" value={localPhone} onChange={(e) => setLocalPhone(e.target.value.replace(/\D/g, ""))} className="flex-1 pl-4 pr-4 py-3 rounded-r-full bg-white border border-gray-200 text-sm text-gray-800 placeholder-gray-400 outline-none" />
      ...
      {openCountry && (
        <ul className="absolute top-full left-0 mt-1 w-72 bg-white border border-gray-200 rounded-2xl shadow-lg max-h-52 overflow-auto text-sm z-50">
          {countries.map((c) => (
            <li key={c.code}>
              <button type="button" onClick={() => { setCountry(c.code); setOpenCountry(false); }} className="w-full flex justify-start gap-3 px-3 py-2 hover:bg-primary-purple/5">
